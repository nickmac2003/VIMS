...
def make_plot(id, force=False):
    ...
    if force or not os.path.exists(plot_path):
        ...
        print(plotter_text % (title, id, id, first, second, score, ramp, correlation), file=plotter)
        plotter.close()
        os.system("gnuplot plotter.gplt")
        os.system("convert -rotate 90 plot.ps %s" % (plot_path,))
    else:
        print("   plot exists, skipping...", plot_path, file=sys.stderr)
...
def create_plot(id):
    ...
    if not os.path.exists(path):
        ...
        print("   data exists, skipping...", id, file=sys.stderr)
    make_plot(id, force)
...
def create_plots():
    ...
    for which, line in  enumerate(sys.stdin):
        ...
        id = list[2]
        print(which, id, file=sys.stderr)
        create_plot(id)
