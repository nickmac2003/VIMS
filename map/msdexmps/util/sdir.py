...
def get_plot_info(id):
    ...
    if os.path.exists(path):
        ...
        if tid != id:
            print("Mismatched ID ", id, tid, file=sys.stderr)
...
def make_entry(id):
    ...
    if info:
        ...
        if spot_id:
            print('<a href=\"' + spot_id + '">', end=' ')
            print('<img src="plots/' + image + '">', end=' ')
            print('</a>')
        else:
            pass
            # print '<img src="plots/' + image + '">'
    else:
        print("Can't open info for", id, file=sys.stderr)
